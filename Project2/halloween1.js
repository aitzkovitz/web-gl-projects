...
function DrawGhost() {
	modelViewMatrix = mat4();
	modelViewMatrix = mult(modelViewMatrix, translate(Math.random()*14-7, Math.random()*6, 0));	
...
function DrawStars(){
	...
	for (var i = 0; i < 20; i++){
		modelViewMatrix = mat4();
		modelViewMatrix = mult(modelViewMatrix, translate( (16 * Math.random()) - 8, 8 * Math.random(), 0));
